...
//				*notice: 4 vertexes
	...
	public void attacking() {	
		...
		f_attack++;
		if(f_attack == attackSpeed*6/8) gp.playerTakeMeleeAttack(attack);
		...
		if (skill2CD != 0) {
			...
			if (skill2EffectiveTime >= 0) {
				if(skill2EffectiveTime > 118) {}
					...
					if(skill2EffectiveTime == 40) gp.playerTakeMeleeAttack(100);
		...
		if (skill3CD != 0) {
			...
			if (skill3EffectiveTime >= 0) {
				if(skill3EffectiveTime > 40) graphics2d.drawImage(skill3[0], x - TILE_SIZE * 3/2, y - TILE_SIZE * 3/2, null);
					...
					if(skill3EffectiveTime == 10) gp.playerTakeMeleeAttack(100);

...
public class Boss4 extends Entity implements GameInterface{
		...
		if(hp > 0) {
			...
			if (!attacking) {
				...
				if (directionAttack == 180) {
					if (f_attack % attackSpeed < attackSpeed/3) {
						...
						if(f_attack % attackSpeed == attackSpeed*5/12) gp.bossTakeAttack(attack);
			...
			if(enemyEntered) {
				if(f_skill % 180 < 10) {
					...
					if(f_skill % 180 == 28) gp.bossTakeAttack(30);

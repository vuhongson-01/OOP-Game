...
//		location for draw image
	...
	public void getPlayerImage() {
			up[0] = setup("/player/boy_up_1", TILE_SIZE, TILE_SIZE);
			up[1] = setup("/player/boy_up_2", TILE_SIZE, TILE_SIZE);
			down[0] = setup("/player/boy_down_1", TILE_SIZE, TILE_SIZE);
			down[1] = setup("/player/boy_down_2", TILE_SIZE, TILE_SIZE);
			left[0] = setup("/player/boy_left_1", TILE_SIZE, TILE_SIZE);
			left[1] = setup("/player/boy_left_2", TILE_SIZE, TILE_SIZE);
			right[0] = setup("/player/boy_right_1", TILE_SIZE, TILE_SIZE);
			right[1] = setup("/player/boy_right_2", TILE_SIZE, TILE_SIZE);
			die[0] = setup("/player/rip", TILE_SIZE, TILE_SIZE);
			
			upAttack[0] = setup("/player/boy_attack_up_1", TILE_SIZE, TILE_SIZE * 2);
			upAttack[1] = setup("/player/boy_attack_up_2", TILE_SIZE, TILE_SIZE * 2);
			downAttack[0] = setup("/player/boy_attack_down_1", TILE_SIZE, TILE_SIZE * 2);
			downAttack[1] = setup("/player/boy_attack_down_2", TILE_SIZE, TILE_SIZE * 2);
			leftAttack[0] = setup("/player/boy_attack_left_1", TILE_SIZE * 2, TILE_SIZE);
			leftAttack[1] = setup("/player/boy_attack_left_2", TILE_SIZE * 2, TILE_SIZE);
			rightAttack[0] = setup("/player/boy_attack_right_1", TILE_SIZE * 2, TILE_SIZE);
			rightAttack[1] = setup("/player/boy_attack_right_2", TILE_SIZE * 2, TILE_SIZE);
			
			pointboard =  setup("/player/pointboard", TILE_SIZE * 4, TILE_SIZE);
			impossible = setup("/player/impossible", TILE_SIZE, TILE_SIZE);
			skill1 = setup("/player/skill1A", TILE_SIZE, TILE_SIZE); 
			skill2 = setup("/player/skill2A", TILE_SIZE, TILE_SIZE); 
			skill3 = setup("/player/skill3A", TILE_SIZE, TILE_SIZE); 
		...
		if (hp > 0) {
			...
			case "up": {
				if (attacking == false) {
					...
					else if (spriteCounter <= 25) { image = up[1];}		
				}
				if (attacking == true) {
					if (spriteCounter <= 5) { image = upAttack[0];}
					else if (spriteCounter <= 25) { 
						...
						image = upAttack[1];
						if (spriteCounter == 20) gp.heroTakeMeleeAttack(damageArea, attack);;
					}
				}
				break;
			}
			case "down": {
				if (attacking == false) {
					if (spriteCounter <= 5) { image = down[0];}
					else if(spriteCounter <= 25) { image = down[1];}	
				}
				if (attacking == true) {
					if (spriteCounter <= 5) { image = downAttack[0];}
					else if (spriteCounter <= 25) { 
			...
			case "left": {
				if (attacking == false) {
					if (spriteCounter <= 5) { image = left[0];}
					else if(spriteCounter <= 25) { image = left[1];}	
				}
				if (attacking == true) {
					if (spriteCounter <= 5) { 
						image = leftAttack[0];}
			...
			case "right": {
				if (attacking == false) {
					if (spriteCounter <= 5) { image = right[0];}
					else if(spriteCounter <= 25) { image = right[1];}	
				}
				if (attacking == true) {
					if (spriteCounter <= 5) { image = rightAttack[0];}
					else if (spriteCounter <= 25) { 
			...
			if (skill1EffectiveTime == 3*60 || (skill1EffectiveTime < 0 && skill1waiting != 0)) {
				if (image == leftAttack[0] || image == leftAttack[1]) {
	
					selfArea[0] = gp.worldx + x + TILE_SIZE + 10;
					selfArea[1] = gp.worldy + y + 10;
					selfArea[2] = gp.worldx + x + TILE_SIZE + 40;
					selfArea[3] = gp.worldy + y + 40;
					graphics2d.drawImage(image, x - tileSize, y, null);
					}
				else if (image == upAttack[1] || image == upAttack[0]){
					selfArea[0] = gp.worldx + x + 10;
					selfArea[1] = gp.worldy + TILE_SIZE + 10;
					selfArea[2] = gp.worldx + x + 40;
					selfArea[3] = gp.worldy + TILE_SIZE + 40;
...
//		skill1
		...
		if (skill1waiting != 0) {
			...
			if (skill1EffectiveTime >= 0) {
				if (skill1EffectiveTime % 12 < 3) {
					...
					damageArea[2] = gp.worldx + x + 54;
					damageArea[3] = gp.worldy + y + 36;
					if (skill1EffectiveTime % 12 == 2) gp.heroTakeMeleeAttack(damageArea, attack/5);
					graphics2d.drawImage(leftAttack[1], x - tileSize, y, null);
				}
					...
					if (skill1EffectiveTime % 12 == 2) gp.heroTakeMeleeAttack(damageArea, attack/5);
					graphics2d.drawImage(upAttack[1], x, y - tileSize, null);
				}
					...
					damageArea[2] = gp.worldx + x + 30;
					damageArea[3] = gp.worldy + y + 24;
					if (skill1EffectiveTime % 12 == 11) gp.heroTakeMeleeAttack(damageArea, attack/5);
					graphics2d.drawImage(downAttack[1], x, y, null);
				}
			}
			...
			graphics2d.drawString(Integer.toString(skill1waiting/60), TILE_SIZE/2 - length/2, SCREEN_HEIGHT-TILE_SIZE/2);
		}
		...
		graphics2d.drawImage(skill2, TILE_SIZE, SCREEN_HEIGHT-TILE_SIZE, null);
...
//		skill2
		...
		if (skill2waiting != 0) {
			...
			graphics2d.drawString(Integer.toString(skill2waiting/60), TILE_SIZE + TILE_SIZE/2 - length/2, SCREEN_HEIGHT-TILE_SIZE/2);
		}
...
//		skill3		
		graphics2d.drawImage(skill3, TILE_SIZE*2, SCREEN_HEIGHT-TILE_SIZE, null);
		...
		if (skill3waiting != 0) {
			...
			graphics2d.drawString(Integer.toString(skill3waiting/60), TILE_SIZE*2 + TILE_SIZE/2 - length/2, SCREEN_HEIGHT-TILE_SIZE/2);
		}

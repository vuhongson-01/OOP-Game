...
public class SpearKnight extends Entity implements GameInterface{
	...
	private int skill3EffectiveTime = 5*60; //during 5s * 60FPS effective
	private int skill3MP = 50; //skill3 need 50 MP
	
	public SpearKnight(GamePanel gp, KeyHandler keyHandler, int[][] map, int firstLocationX, int firstLocationY) {
		this.gp = gp;
...
//				*notice: 4 vertexes
	...
	public void attacking() {	
		...
		f_attack++;
		if(f_attack == attackSpeed*6/8) gp.heroTakeMeleeAttack(damageArea, attack);
	...
	public void draw(Graphics2D graphics2d) {	
		...
		case "down": {
			...
			if (attacking == true) {
				...
				damageArea[1] = y + gp.worldy + TILE_SIZE;
				damageArea[2] = x + gp.worldx + TILE_SIZE;
				damageArea[3] = y + gp.worldy + 2 * TILE_SIZE + TILE_SIZE;			
		...
		if (skill2CD != 0) {
			...
			if (skill2EffectiveTime >= 0) {
				if(skill2EffectiveTime > 118) {}
					...
					if(skill2EffectiveTime == 40) gp.heroTakeMeleeAttack(damageArea, 100);
...
//****************************	skill 3	**************************************	
		...
		if (skill3CD != 0) {
			...
			if (skill3EffectiveTime >= 0) {
				if(skill3EffectiveTime > 40) graphics2d.drawImage(skill3[0], x - TILE_SIZE * 3/2, y - TILE_SIZE * 3/2, null);
					...
					if(skill3EffectiveTime == 10) gp.heroTakeMeleeAttack(damageArea, 100);
